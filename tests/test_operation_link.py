...
class TestLinkType:
    ...
    def test_link_type_values(self) -> None:
        """Test that LinkType has the expected values."""
        assert LinkType.PLANNED_OPERATION == "planned_operation"
        assert LinkType.BUDGET == "budget"

    def test_link_type_is_str_enum(self) -> None:
        """Test that LinkType values can be used as strings."""
        assert str(LinkType.PLANNED_OPERATION) == "planned_operation"
        assert str(LinkType.BUDGET) == "budget"

    def test_link_type_from_string(self) -> None:
        """Test that LinkType can be created from string."""
        assert LinkType("planned_operation") == LinkType.PLANNED_OPERATION
        assert LinkType("budget") == LinkType.BUDGET


class TestOperationLink:
    """Tests for the OperationLink NamedTuple."""

    def test_create_operation_link_minimal(self) -> None:
        """Test creating an OperationLink with required fields only."""
        link = OperationLink(
            operation_unique_id=123,
            linked_type=LinkType.PLANNED_OPERATION,
            linked_id=456,
            iteration_date=datetime(2024, 1, 15),
        )
        assert link.operation_unique_id == 123
        assert link.linked_type == LinkType.PLANNED_OPERATION
        assert link.linked_id == 456
        assert link.iteration_date == datetime(2024, 1, 15)
        assert link.is_manual is False
        assert link.notes is None

    def test_create_operation_link_full(self) -> None:
        """Test creating an OperationLink with all fields."""
        link = OperationLink(
            operation_unique_id=123,
            ...
            is_manual=True,
            notes="Manually linked because description didn't match",
        )
        assert link.operation_unique_id == 123
        assert link.linked_type == LinkType.BUDGET
        assert link.linked_id == 789
        assert link.iteration_date == datetime(2024, 2, 1)
        assert link.is_manual is True
        assert link.notes == "Manually linked because description didn't match"

    def test_operation_link_is_named_tuple(self) -> None:
        """Test that OperationLink behaves as a NamedTuple."""
        link = OperationLink(
        ...
        # NamedTuple should support _replace
        new_link = link._replace(is_manual=True)
        assert new_link.is_manual is True
        assert link.is_manual is False  # Original unchanged
