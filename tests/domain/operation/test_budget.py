...
def budget_no_period() -> Budget:
    ...
    return Budget(
        ...
        amount=Amount(-200.0),
        category=Category.GROCERIES,
        date_range=DateRange(date(2023, 1, 1), relativedelta(days=30)),
    )
...
class TestBudgetArchived:
    """Tests for the is_archived field on Budget."""

    def test_default_not_archived(self) -> None:
        """Test that budgets are not archived by default."""
        budget = Budget(
            record_id=1,
            description="Test Budget",
            amount=Amount(-100.0),
        ...
        assert budget.is_archived is False

    def test_create_archived(self) -> None:
        """Test creating an archived budget."""
        budget = Budget(
            record_id=1,
            description="Test Budget",
            amount=Amount(-100.0),
        ...
        assert budget.is_archived is True

    def test_replace_with_is_archived(self) -> None:
        """Test replace() can set is_archived."""
        budget = Budget(
            record_id=1,
            description="Test Budget",
            amount=Amount(-100.0),
        ...
        archived = budget.replace(is_archived=True)
        assert archived.is_archived is True
        assert budget.is_archived is False

    def test_replace_preserves_is_archived(self) -> None:
        """Test replace() preserves is_archived when not specified."""
        budget = Budget(
            record_id=1,
            description="Test Budget",
            amount=Amount(-100.0),
        ...
        replaced = budget.replace(description="Updated")
        assert replaced.is_archived is True

    def test_replace_invalid_is_archived(self) -> None:
        """Test replace() raises TypeError for invalid is_archived."""
        budget = Budget(
            record_id=1,
            description="Test Budget",
            amount=Amount(-100.0),
        ...
        with pytest.raises(TypeError, match="is_archived must be bool"):
            budget.replace(is_archived="true")

    def test_equality_considers_is_archived(self) -> None:
        """Test that equality considers is_archived."""
        budget1 = Budget(
            record_id=1,
            description="Test Budget",
            amount=Amount(-100.0),
...
class TestBudgetReplaceTypeErrors:
    ...
    def test_replace_invalid_time_range(self, budget: Budget) -> None:
        ...
        with pytest.raises(TypeError, match="date_range must be DateRangeInterface"):
            budget.replace(date_range="2023-01-01")
...
class TestBudgetSplitAt:
    ...
    def test_split_at_with_new_duration(self) -> None:
        ...
        assert continuation.date_range.duration == relativedelta(months=2)
