...
def generate_enroll_html(bookmarklet_code: str) -> str:
    """Generate the enroll bookmarklet installation page."""
    escaped_code = html.escape(bookmarklet_code, quote=True)
...
<head>
    <meta charset="UTF-8">
    <title>Swile Enroll - Bookmarklet Installation</title>
    <style>
        body {{
            font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif;
            max-width: 600px;
...
<body>
    <h1>Swile Enroll</h1>
    <p>Connect Swile to Budget Forecaster so it can sync on its own.</p>

    <h2>Installation</h2>
    <p>Drag this button to your bookmarks bar:</p>

    <a class="bookmarklet" href="{escaped_code}">Swile Enroll</a>
    ...
    <div class="step">
        <span class="step-number">2</span>
        Click the "Swile Enroll" bookmarklet; it copies your token to the clipboard
    </div>

    <div class="step">
        <span class="step-number">3</span>
        Paste the token into Budget Forecaster &rarr; Settings &rarr; Swile
    </div>
</body>
</html>
"""
